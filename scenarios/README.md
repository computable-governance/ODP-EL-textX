...
| `ecommerce/ecommerce_scenario.el` | Historical | v2 | 2026-06-01 (single commit) | ISO/IEC 15414 Annex B e-commerce validation case, non-clinical. Does not parse — see docs/CONCEPTS_INDEX.md (AM-93 note). Never iterated on. |
...
| `probes/transfer_probe.el` | Probe | v2 | 2026-09-14 (AM-82) | First and only live exercise of the `transfer` DeonticEffect (§6.4.7/§7.8.7) anywhere in this repo's scenarios — `referral_scenario.el` never uses it. One Burden happy-path transfer (roleA → roleB) plus every Rule T9 skip case in one file (no `from_role`, ambiguous `from_role`, unfilled `to_role`, Permit-kind token), all gated behind one strict burden to also exercise T9's `strict_burden_blocks()` guard. Role membership is assigned in Python (`el_engine.enroll()`), not in the grammar — there is no actor-to-role binding construct. 8 tests passing (`tests/test_hybrid_t9_transfer.py`). |
| `terms_of_engagement/external_agent_access_scenario.el` | Probe | v2 | 2026-09-25 (single commit) | Tier-1 terms of engagement for external AI agents (unilateral contract community; security, normative and deontic policies side by side). Dead-ended at 4 worlds until AM-99a part 2 (Rule T11 fires action-emitted triggers in the static verifier); now 2444 worlds, EF true for all three burdens (`tests/test_am99a_t11_event_firing.py`). Bounded response property (AM-99a part 3, `tests/test_am99a_bounded_response.py`): `refusalRecordBurden` (strict) holds; `refusalReviewBurden` and `incidentNotificationBurden` (eventual) fail with EF true. Candidate for promotion to Demo. |
| `terms_of_engagement/public_data_portal_scenario.el` | Probe | v2 | 2026-09-25 (single commit) | Sibling of external_agent_access_scenario.el, modelled on the Medicare portal incident: an external agent reading a public statistics portal, with non-public files embargoed from entry. Same verified reading from both builders. |
