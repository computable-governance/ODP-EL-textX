...
@app.post(
    ...
    description=(
        ...
        "For each firing (AM-104, whether or not the response has "
        "creates_burden): grants creates_burden, if set, to obligates via the "
        "same general-purpose token_from_spec() path every other fresh-grant "
        "call uses; logs an informational escalate_to entry (no token/event "
        "fired for it — emits/§8.4 outbound notification stays deferred, see "
        "docs/CONCEPTS_INDEX.md) and a line naming the response_kind; and, for "
        "response_kind terminate, revokes the violator's Authorizations whose "
        "authority is obligates (allowed during a strict freeze). Idempotent: "
        "fires exactly once per violated instance, never again — even after "
        "its created burden is discharged. Tick only advances when at least one response actually "
